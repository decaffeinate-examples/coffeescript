...
class LineMap {
  ...
  add(column, ...rest) {
    const [sourceLine, sourceColumn] = Array.from(rest[0]), val = rest[1], options = val != null ? val : {};
    if (this.columns[column] && options.noReplace) { return; }
    return this.columns[column] = {line: this.line, column, sourceLine, sourceColumn};
  }
...
var SourceMap = (function() {
  let VLQ_SHIFT = undefined;
  let VLQ_CONTINUATION_BIT = undefined;
  let VLQ_VALUE_MASK = undefined;
  let BASE64_CHARS = undefined;
  ...
  // effect.
    add(sourceLocation, generatedLocation, options) {
      ...
      const [line, column] = Array.from(generatedLocation);
  ...
  // code.
    sourceLocation(...args) {
      let lineMap;
      let [line, column] = Array.from(args[0]);
  ...
  // set "sources" and "file", respectively.
    generate(options, code = null) {
      ...
      for (let lineNumber = 0; lineNumber < this.lines.length; lineNumber++) {
        ...
        if (lineMap) {
          for (let mapping of Array.from(lineMap.columns)) {
            if (mapping) {
  ...
  // Produce the canonical JSON object format for a "v3" source map.
      ...
      if (options.inlineMap) { v3.sourcesContent = [code]; }
