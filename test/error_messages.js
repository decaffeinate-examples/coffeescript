...
const assertErrorFormat = (code, expectedErrorFormat) => throws((() => CoffeeScript.run(code)), (err) => {
  err.colorful = false;
...
test('compiler error formatting with mixed tab and space', () => assertErrorFormat(`\
\t  if a
\t  test\
`,
`\
...
`);
  ...
  // Make sure the line number reported is line 3 (the original Coffee source)
  // and not line 6 (the generated JavaScript).
  return eq(/at testCompiledStringStackTraceLineNumber.*:(\d):/.exec(error.stack.toString())[1], '3');
});
