...
test('bound generator', () => {
  ...
  return eq(obj, obj.nested().next().value.next().value.next().value);
});
...
test('yield in if statements', () => {
  const x = (function* () { if ((yield 2) === 1) { return 3; } return 4; }());
...
test('symbolic operators has precedence over the `yield`', () => {
  ...
  const operators = [...Array.from(symbolic), '=', ...Array.from(compound), ...Array.from(relations)];
...
test('from as a destructured, aliased object variable name in a for loop declaration', () => {
  ...
  for (const { b: from } of Array.from(a)) {
    out.push(from);
  }
